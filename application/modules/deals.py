...
def deals(filter, page, error, prev_sort, sort, order):
    ...
    if search_form.validate_on_submit():
        ...
        # If the user is looking for an email, only search the email column.
            ...
            deals = deals.filter(
                Deals.associated_contact.like(f"%{user_search}%"))
        # If the user isn't looking for an email or number definitively then search all relevant columns.
        else:
            deals = deals.filter(Deals.associated_contact.like(f"%{user_search}%") | Deals.name.like(
                f"%{user_search}%") | Deals.deal_id.like(f"%{user_search}%") | Deals.goal.like(f"%{user_search}%"))
...
def order_deals(sort, order, deals):
    ...
    if sort == "name":
        ...
        if order == "ASC":
            deals = deals.order_by(Deals.goal)
        else:
            deals = deals.order_by(Deals.goal.desc())
